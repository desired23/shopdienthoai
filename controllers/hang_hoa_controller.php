...
//Hiện hàng hóa trong home
function show_hang_hoa_home()
{
    $hang_hoa = hang_hoa_home();
    $loai_hang = loai_hang_hoa_home();
    $top10s = top10_hang_hoa_home();
    render('home', ['hang_hoa' => $hang_hoa, 'loai_hang' => $loai_hang, 'top10s' => $top10s]);
}

//Hiện hàng hóa trong home
function show_product_detail($mahh)
{
    $get_info_pro = hang_hoa_one($mahh);
    extract($get_info_pro);
    $sp_related = hang_hoa_related($ma_loai, $mahh);
    $loai_hang = loai_hang_hoa_home();
    $top10s = top10_hang_hoa_home();
    render('product_detail', ['get_info_pro' => $get_info_pro, 'sp_related' => $sp_related, 'loai_hang' => $loai_hang, 'top10s' => $top10s]);
}
//Hiện hàng hóa trong product
function show_hang_hoa_product($ma_loai)
{
    $hang_hoa = hang_hoa_danh_muc($ma_loai);
    $loai_hang = loai_hang_hoa_home();
    $top10s = top10_hang_hoa_home();
        $cate_name = get_cate_name($ma_loai);
        render('sanpham', ['hang_hoa' => $hang_hoa, 'loai_hang' => $loai_hang, 'top10s' => $top10s, 'cate_name' => $cate_name ]);
}

function show_result ($key)
{
    $hang_hoa = searchfunc($key);
    $loai_hang = loai_hang_hoa_home();
    $top10s = top10_hang_hoa_home();

        render('search_result', ['hang_hoa' => $hang_hoa, 'loai_hang' => $loai_hang, 'top10s' => $top10s, 'key' => $key]);
}
